...
Page({
  ...
  hqlist: function (options) {
    ...
    wx.getStorage({
      ...
      success: function (res) {
        ...
        wx.request({
          url: "http://vczyh.top/wxapp/v1.0/user/words/appear/list/DESC/" + res.data.result,
          // data: data,
          method: 'POST',
          header: {
        ...
        wx.request({
          ...
          success: function (res) {
            ...
            if (res.data.message === "success") {
              ...
              // 调用获取错误单词接口实现数据刷新
              wx.request({
                url: "http://vczyh.top/wxapp/v1.0/user/words/appear/list/DESC/" + ineedtoken,
                // data: data,
                method: 'POST',
                header: {
                  // "Content-Type":"application/json"
  ...
  gotoitem: function (options) {
    ...
    this.setData({
      showxx: true,
      showjb: false,
      storageContent: options.currentTarget.dataset.word.word
    })
  },
  /**
   * 生命周期函数--监听页面加载
